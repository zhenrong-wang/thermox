...
- [Validation Campaign and Report Architecture](docs/job-validation-report-architecture.md)
...
- First-class `thermox_service` application module with versioned validate, steady, and transient
  ...
  `thermox.result/v6` JSON with complete execution provenance and graph-native steady/transient
  values. Its `thermox.job/v19` workflow adds Team-scoped idempotent execution, leased worker claims,
...
- Provider-neutral validation-series artifacts bind immutable measured, computational, derived, or
  ...
  per-sample verdicts, source provenance, uncertainty, and limitations afterward. Compact Result
  Summary v5 verdicts make reference agreement visible in job history without treating a mismatch
...
- A leakage-guarded engineering-study service calibrates designated baseline cases, freezes the
  canonical fitted model, runs independent steady or transient prediction cases, and reports per-observation
  and aggregate normalized validation residuals. A versioned
  `thermox.engineering_study/v2` declaration and thin `thermox_cli study --input ...` adapter make
...
Finite full-system steps that miss the configured backward-error tolerance receive at most two
residual-correction solves through the same backend. Results expose correction attempts and
successful recoveries; already accurate direct solves incur no additional solve.
...
transient definitions through `thermox.expression_component/v5`, including declared internal
states, fixed-topology operating modes, component-owned event surfaces, atomic expression resets,
...
definitions under `thermox.job/v19`. Project-owned
