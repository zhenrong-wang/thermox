...
- a backend-neutral `state_ph_derivatives` result containing the state and
  temperature, density, internal-energy, and entropy partials with respect to pressure and
  enthalpy;
...
property backend. The built-in ideal-gas package evaluates the eight partials from its closed-form
...
and derives all eight partials from them. Neighboring points are selected from the same phase where
...
user-declared isentropic closures. A third-party backend can replace the fallback simply by
