...
into `thermox.index1-model-family/v3`. Every member retains its complete A/B/C/D model and nonlinear
...
cross-regime interpolation. Separately declared held-out cases are independently linearized and
compared entry-by-entry with training-only interpolated matrices under explicit absolute and
relative tolerances.
