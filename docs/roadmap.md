...
7. Safe custom equation DSL; no arbitrary user Python in API process.
   ...
   - publication-time compound dimension inference for arithmetic, derivative states, powers,
     ...
     dimensions; ✅
   - constrained same-port p-h temperature, density, internal-energy, entropy, and explicitly
     two-phase vapor-quality functions with derived
   ...
   - entropy-backed isentropic and quality-backed two-phase project closures are covered; approval
     policy and further derivative-backed property functions remain follow-ons.
...
10. Map validation and interpolation quality controls.
    ...
    - dimensioned v3 Study operating envelopes for performance-map coordinates, correlation
      ...
      violation errors. ✅
    - structured v3 result-summary engineering evidence with signed lower/upper margins, limiting
      bound identification, transient-extremum support, durable PostgreSQL encoding, and web result
      presentation; absolute and event-relative graph-native windows with exact boundary
      interpolation, mean/RMS reductions, and window-compatible multi-signal comparisons. ✅
