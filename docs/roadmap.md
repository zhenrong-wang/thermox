...
- graph-native `thermox.result/v6` steady/transient/event values for every connector domain,
...
- transport-neutral `thermox.job/v19` lifecycle with Team-scoped idempotency, leased worker claims,
...
19. Add opt-in bounded profile-likelihood intervals with nuisance-parameter re-optimization,
    ...
    thin CLI access. ✅
20. Carry data reconciliation through the common `thermox.job/v19` idempotent queue, leased
