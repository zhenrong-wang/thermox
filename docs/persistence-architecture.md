...
All gates are now in place. `thermox.job/v12` defines the Team-owned job lifecycle and idempotent
...
submission. `thermox.job/v12` captures the immutable source provenance and composed snapshot, so
