...
All gates are now in place. `thermox.job/v15` defines the Team-owned job lifecycle and idempotent
...
submission. `thermox.job/v15` captures the immutable source provenance and composed snapshot, so
