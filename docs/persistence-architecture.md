...
All gates are now in place. `thermox.job/v19` defines the Team-owned job lifecycle and idempotent
submission, leased worker claim, optimistic terminal publication, and queued cancellation. The application
service writes a checksummed `thermox.result/v6` JSON artifact before publishing a succeeded job.
...
submission. `thermox.job/v19` captures the immutable source provenance and composed snapshot, so
...
finite interval bounds before persistence. Job schema v15 snapshots the applied coordinate bounds
alongside the exact performance-map, correlation, or regime-map payload, and its fingerprint
includes every bound and inclusion flag, so
