...
| Calibration/study service | Immutable Study and calibration revisions bind exact topology, cases, artifacts, intent, dataset split, and output projections; calibration executes as a durable job and distinguishes measurement identifiability from prior-regularized posterior uncertainty | Preserve; replace the reference optimizer only behind the stable evidence contract |
